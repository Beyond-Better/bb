...
const loadStoredState = () => {
  ...
  let selectedProjectId = params.get('projectId');

  // If not in URL, check localStorage
  if (!selectedProjectId) {
    selectedProjectId = localStorage.getItem('bb_projectId');
  }
...
export function useProjectState(appState: Signal<AppState>) {
  async function loadProjects() {
    ...
    try {
      const response = await apiClient.get<{ projects: Project[] }>('/api/v1/project');
      ...
      if (response) {
        ...
        projectState.value = {
          ...
          selectedProjectId: projectExists ? projectState.value.selectedProjectId : null,
          loading: false,
        };
      }
    } catch (error) {
  ...
  async function createProject(project: Omit<Project, 'projectId'>) {
    ...
    projectState.value = { ...projectState.value, loading: true, error: null };
    try {
      const response = await apiClient.post<{ project: Project }>('/api/v1/project', project);
      if (response) {
        projectState.value = {
          ...projectState.value,
  ...
  async function updateProject(projectId: string, updates: Partial<Omit<Project, 'projectId'>>) {
    ...
    projectState.value = { ...projectState.value, loading: true, error: null };
    try {
      const response = await apiClient.put<{ project: Project }>(`/api/v1/project/${projectId}`, updates);
      if (response) {
        projectState.value = {
          ...projectState.value,
  ...
  async function deleteProject(projectId: string) {
    ...
    try {
      ...
      projectState.value = {
        ...
        selectedProjectId: projectState.value.selectedProjectId === projectId
          ? null
  ...
  async function migrateAndAddProject(projectPath: string): Promise<void> {
    ...
    projectState.value = { ...projectState.value, loading: true, error: null };
    try {
      await apiClient.post('/api/v1/project/migrate', { projectPath });
  ...
  function getSelectedProject(): Project | null {
    if (!projectState.value.selectedProjectId) return null;
    ...
    while (retryCount < maxRetries) {
      ...
      projectState.value = {
        ...
        projects: projectState.value.projects.map((p) => 
          p.projectId === projectId ? { ...p, stats } : p
        ),
      };
      } catch (error) {
        console.error(`Failed to update project stats (attempt ${retryCount + 1}): ${(error as Error).message}`);
        retryCount++;
