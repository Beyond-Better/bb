...
export default function MCPServerItem({
					...
					<div>
						...
						<input
							...
							onChange={(e) =>
								handleInputChange(
									'args',
									(e.target as HTMLInputElement).value.split(',').map((arg) => arg.trim()).filter(
										(arg) => arg,
									),
								)}
							class='mt-1 form-input block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:text-white text-sm px-3 py-2'
	...
	return (
		<div
			class={`border ${
				isGlobal ? 'border-gray-200 dark:border-gray-700' : 'border-green-100 dark:border-green-900'
			} rounded-md p-3 bg-white dark:bg-gray-900`}
		>
			<div class='flex items-center justify-between'>
