							...
							<div class='space-y-3'>
								<div>
									...
									<input
										...
										onChange={(e) =>
											handleNewServerChange('id', (e.target as HTMLInputElement).value, true)}
								...
								<div>
									...
									<input
										...
										onChange={(e) =>
											handleNewServerChange('name', (e.target as HTMLInputElement).value, true)}
										class='mt-1 form-input block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:text-white text-sm px-3 py-2'
										placeholder='Slack'
									/>
								</div>
								...
								<div>
									...
									<input
										...
										onChange={(e) =>
											handleNewServerChange(
												'args',
												(e.target as HTMLInputElement).value.split(',').map((arg) => arg.trim())
													.filter((arg) => arg),
												true,
											)}
										class='mt-1 form-input block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:text-white text-sm px-3 py-2'
								...
								<div>
									...
									<textarea
										rows={3}
										value={Object.entries(newServer.global.env || {}).map(([key, value]) =>
											`${key}=${value}`
										).join('\n')}
										onChange={(e) => {
							...
							<div class='border border-gray-200 dark:border-gray-700 rounded-md p-4 bg-gray-50 dark:bg-gray-800 mb-4'>
								...
								<div class='space-y-3'>
									...
									<div>
										...
										<input
											...
											onChange={(e) =>
												handleNewServerChange(
													'args',
													(e.target as HTMLInputElement).value.split(',').map((arg) =>
														arg.trim()
													).filter((arg) => arg),
													false,
												)}
									...
									<div>
										...
										<textarea
											rows={3}
											value={Object.entries(newServer.project.env || {}).map(([key, value]) =>
												`${key}=${value}`
											).join('\n')}
											onChange={(e) => {
												const envVars: Record<string, string> = {};
