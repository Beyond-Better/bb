    ...
    <div className='mcp-servers-overview'>
      ...
      {servers.length > 0 ? (
        <div className='space-y-3'>
          {servers.map(server => (
            <div key={server.id} className='bg-gray-50 dark:bg-gray-800 rounded-md p-3 flex justify-between items-center'>
              <div className='flex flex-col'>
                <div className='flex items-center'>
                  ...
                  {server.id !== server.name && server.name && (
                    <span className='ml-2 text-xs text-gray-500 dark:text-gray-400'>({server.id})</span>
                  )}
                </div>
                <span className='text-xs text-gray-500 dark:text-gray-400'>{server.command} {server.args?.join(' ')}</span>
              </div>
