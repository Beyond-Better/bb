...
export default function MCPConfigModal({
  ...
  const handleSaveServer = () => {
    ...
    if (editingServer) {
      ...
      setEditedServers(editedServers.map(server => 
              ...
              <div className='space-y-4'>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => handleUpdateServer('id', (e.target as HTMLInputElement).value)}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => handleUpdateServer('name', (e.target as HTMLInputElement).value)}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => handleUpdateServer('command', (e.target as HTMLInputElement).value)}
                    className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 ${
                ...
                <div>
                  ...
                  <textarea
                    rows={5}
                    value={formatEnvVars(showSensitive ? (editingServer || newServer).env : undefined)}
...
SLACK_TEAM_ID=T12345'
                  />
                  {!showSensitive && (editingServer || newServer).env && Object.keys((editingServer || newServer).env || {}).length > 0 && (
                    <p className='mt-1 text-xs text-amber-600 dark:text-amber-400'>
                      <span className='font-medium'>Note:</span> Environment variable values are hidden. Click 'Show Values' to view and edit.
                    </p>
                  )}
                </div>
              </div>
              ...
              {editedServers.length > 0 ? (
                <div className='space-y-4'>
                  {editedServers.map(server => (
                    <div key={server.id} className='bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4'>
                        ...
                        <div>
                          ...
                          <div className='mt-1 text-sm text-gray-500 dark:text-gray-400'>
                            ...
                            <div className='flex items-center'>
                              ...
                              <span className='ml-1 font-mono'>{server.command}</span>
                            </div>
                            {server.args && server.args.length > 0 && (
                              <div className='flex items-start'>
                                <span className='font-medium'>Args:</span>
                                <span className='ml-1 font-mono'>{server.args.join(', ')}</span>
                              </div>
                            )}
                            {server.env && Object.keys(server.env).length > 0 && (
                              <div>
                                ...
                                <span className='ml-1'>
                                  {Object.keys(server.env).length} variable{Object.keys(server.env).length !== 1 ? 's' : ''}
                                </span>
                              </div>
                            )}
                          </div>
