					...
					: (
						<ul className='space-y-2'>
							{conversationListState.value.conversations
								.sort((a, b) => {
									// Move selected item to top of the list
									//if (a.id === conversationListState.value.selectedId) return -1;
									//if (b.id === conversationListState.value.selectedId) return 1;
									return new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime();
								})
								.map((conv) => (
									<li
										key={conv.id}
										className={`p-4 hover:bg-gray-50 cursor-pointer transition-all duration-200 rounded-lg border ${
									...
									>
										...
										<div className='grid grid-cols-2 gap-1.5 mt-1 text-xs text-gray-500'>
