						...
						<input
							...
							if (e.key === 'Enter' && showSuggestions.value && suggestions.value.length > 0) {
								...
								if (selectedIndex.value !== undefined && selectedIndex.value >= 0) {
									...
									console.log('Closing suggestions, resetting selection');
									batch(() => {
							...
							if (showSuggestions.value && suggestions.value.length > 0) {
								if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
									...
									const newIndex = e.key === 'ArrowDown'
										? ((selectedIndex.value === undefined ? -1 : selectedIndex.value) + 1) % suggestions.value.length
										: selectedIndex.value === undefined || selectedIndex.value <= 0
										? suggestions.value.length - 1
										: selectedIndex.value - 1;
									console.log('Changing selection:', { from: selectedIndex.value, to: newIndex });
selectedIndex.value = newIndex;
						...
						onInput={async (e) => {
								...
								if (value.trim()) {
									...
									try {
										const response = await appState.value.apiClient?.suggestFiles(value, project?.projectId || '');
										batch(() => {
											console.log('Loading new suggestions');
							...
							aria-activedescendant={selectedIndex.value !== undefined ? `suggestion-${selectedIndex.value}` : undefined}
						/>
					...
					{showSuggestions.value && suggestions.value.length > 0 && (
						...
						class='absolute z-10 w-full mt-1 bg-white dark:bg-gray-800 shadow-lg rounded-md py-1 text-base ring-1 ring-black dark:ring-white ring-opacity-5 dark:ring-opacity-10 overflow-auto focus:outline-none sm:text-sm max-h-60 scroll-smooth'>
							...
							//console.log('Rendering suggestion:', { index: i, selectedIndex: selectedIndex.value, isSelected });
							return (
								<li
									id={`suggestion-${i}`}
									key={suggestion.path}
									...
									class={`cursor-pointer py-2 px-3 ${isSelected ? 'bg-blue-600 text-white' : 'text-gray-900 dark:text-gray-100 hover:bg-blue-600 hover:text-white hover:no-underline'}`}
									onClick={() => {
								...
								>
									<div class='flex items-center'>
										...
										<span class={`ml-2 truncate text-sm ${isSelected ? 'text-blue-100' : 'text-gray-500'}`}>
											({suggestion.path})
										</span>
									</div>
								</li>
