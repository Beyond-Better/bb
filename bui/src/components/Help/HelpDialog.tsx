...
import { useTransition, useFadeTransition } from '../../hooks/useTransition.ts';
...
const HELP_SECTIONS: HelpSection[] = [
      ...
      <div className="space-y-2">
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center space-x-1">
            <kbd className="px-2 py-1 bg-gray-100 border border-gray-300 rounded text-sm shadow-sm">Shift</kbd>
    ...
    content: (
      <div className="space-y-3">
        <p className="text-gray-600">
          Messages can be expanded or collapsed using the spacebar or by clicking.
          Use Tab to move between interactive elements in messages.
        </p>
        <p className="text-gray-600">
          Each message shows its role (user, assistant, tool) and content.
          Tool messages show the specific action being taken.
        </p>
      </div>
    )
  },
  {
    ...
    content: (
      <div className="space-y-3">
        <p className="text-gray-600">
          Conversations get more expensive as they grow longer. Consider starting new conversations
          or using the conversation summary button to reduce token usage.
        </p>
        <div className="space-y-2">
  ...
  },
  {
    ...
    content: (
      <div className="space-y-3">
        <p className="text-gray-600">
          BB can work with multiple projects. The current project directory is shown in the header
          and can be changed to switch between projects.
        </p>
        <p className="text-gray-600">
  ...
  },
  {
    ...
    content: (
      <div className="space-y-3">
        <p className="text-gray-600">
          The header shows API connection status and total token usage.
          A banner appears when Claude is processing your request.
        </p>
        <p className="text-gray-600">
      ...
      <div className="space-y-3">
        <dl className="space-y-3">
          <div>
            ...
            <dd className="text-gray-600">The root directory and all files BB is working with. BB can work with multiple projects.</dd>
          </div>
          <div>
            <dt className="font-medium text-gray-700">Conversation</dt>
            <dd className="text-gray-600">A complete interaction session with BB, containing multiple statements.</dd>
          </div>
          <div>
            <dt className="font-medium text-gray-700">Statement</dt>
            <dd className="text-gray-600">All messages from a user query through the assistant's answer, including tool messages.</dd>
          </div>
          <div>
          ...
          <div>
            ...
            <dd className="text-gray-600">Requests from Claude for BB to take action (input) and the results of those actions (output).</dd>
          </div>
          <div>
            <dt className="font-medium text-gray-700">Auxiliary</dt>
            <dd className="text-gray-600">Additional chat messages for conversation management (titles, commits) and objectives.</dd>
          </div>
          <div>
            <dt className="font-medium text-gray-700">Objectives</dt>
            <dd className="text-gray-600">The focus given to Claude. Each statement has a new objective, plus an overall conversation objective.</dd>
          </div>
          <div>
            <dt className="font-medium text-gray-700">Tools</dt>
            <dd className="text-gray-600">The core functionality that enables BB to take action - the code that performs tasks.</dd>
          </div>
          <div>
            <dt className="font-medium text-gray-700">Tokens</dt>
            <dd className="text-gray-600">The "cost unit" for working with Claude. Longer conversations use more tokens.</dd>
          </div>
        </dl>
      </div>
    )
