...
export function Settings(): JSX.Element {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.log('Saving updates:', updates.map(([key, value]) => [
        key,
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <form onSubmit={(e) => { e.preventDefault(); handleSave(); }} className="space-y-6">
        <div>
          ...
          />
          {errors.myPersonsName && (
            <p className="mt-1 text-sm text-red-500">{errors.myPersonsName}</p>
          )}
        </div>
        ...
        <div>
          ...
          />
          {errors.myAssistantsName && (
            <p className="mt-1 text-sm text-red-500">{errors.myAssistantsName}</p>
          )}
        </div>
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              className={`w-full px-3 py-2 border rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-white ${
                errors['api.llmKeys.anthropic'] ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'
              }`}
        ...
        <div>
          ...
          />
          {errors['api.maxTurns'] && (
            <p className="mt-1 text-sm text-red-500">{errors['api.maxTurns']}</p>
          )}
        </div>
      ...
      <ConfirmDialog
        ...
        title="Restart API"
        message="The API needs to be restarted to apply these changes. Any ongoing conversations will be interrupted. Do you want to restart now?"
