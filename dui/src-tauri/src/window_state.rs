...
static SAVE_HANDLE: OnceCell<tokio::sync::Mutex<Option<tauri::async_runtime::JoinHandle<()>>>> = OnceCell::new();
        ...
        let scale_factor = if let Some(handle) = app_handle {
            ...
            if let Ok(monitors) = handle.primary_monitor() {
                if let Some(monitor) = monitors {
                    if debug_enabled {
                        info!("[DEBUG] Got primary monitor scale factor: {}", monitor.scale_factor());
                    }
...
impl Default for WindowState {
    fn default() -> Self {
        ...
        if debug_enabled {
            ...
            info!("[DEBUG] - Logical size: {}x{}", logical_width, logical_height);
...
fn validate_window_state(state: &WindowState, _window: Option<&WebviewWindow>) -> WindowState {
    ...
    if debug_enabled {
        ...
        info!("[DEBUG] - Size (physical): {}x{}", state.width, state.height);
        info!("[DEBUG] - Position (physical): {:?},{:?}", state.x, state.y);
        info!("[DEBUG] - Scale factor: {}", state.scale_factor);
        info!("[DEBUG] - Logical size would be: {}x{}", state.width / state.scale_factor, state.height / state.scale_factor);
        if let (Some(x), Some(y)) = (state.x, state.y) {
            info!("[DEBUG] - Logical position would be: {}x{}", x / state.scale_factor, y / state.scale_factor);
        }
    }
    ...
    if let (Some(x), Some(y)) = (validated.x, validated.y) {
        ...
        if x < -MAX_DIMENSION || x > MAX_DIMENSION || y < -MAX_DIMENSION || y > MAX_DIMENSION {
            if debug_enabled {
                info!("[DEBUG] Window position ({},{}) is out of reasonable bounds, will center", x, y);
            }
            validated.x = None;
            validated.y = None;
        } else if debug_enabled {
            info!("[DEBUG] Window position ({},{}) is within reasonable bounds", x, y);
        }
    }
...
#[tauri::command]
pub async fn setup_window_state_handler(window_label: String, app_handle: tauri::AppHandle) -> Result<(), String> {
...
pub fn setup_window_state_handler_internal(window: &WebviewWindow) {
    let debug_enabled = get_dui_debug_mode();
    if debug_enabled {
        info!("[DEBUG] Setting up window state handlers for window: {}", window.label());
    }
    let window_clone = window.clone();
    window.on_window_event(move |event| {
        if debug_enabled {
            match event {
                tauri::WindowEvent::Moved(_) => info!("[DEBUG] Move event received for window: {}", window_clone.label()),
                tauri::WindowEvent::Resized(_) => info!("[DEBUG] Resize event received for window: {}", window_clone.label()),
                _ => {}
            }
        }
            ...
            tauri::WindowEvent::ScaleFactorChanged { scale_factor, .. } => {
                ...
                if debug_enabled {
                    ...
                    if let Ok(current_size) = window_clone.outer_size() {
                        info!("[DEBUG] - Current window size (physical): {}x{}", current_size.width, current_size.height);
                    }
                    if let Ok(current_pos) = window_clone.outer_position() {
                        info!("[DEBUG] - Current window position (physical): {}x{}", current_pos.x, current_pos.y);
                    }
                }
                if debug_enabled {
...
pub async fn load_window_state(
    ...
    if debug_enabled {
        info!("[DEBUG] Loading window state with scale factor: {}", actual_scale_factor);
    }
    let debug_enabled = get_dui_debug_mode();
    if debug_enabled {
    ...
    let store = app_handle.store("bb-window-state.json")
        .map_err(|e| {
            if debug_enabled {
    ...
    let result = match state {
        Some(state) => {
            ...
            if use_logical_size.unwrap_or(false) {
                if debug_enabled {
                    info!("[DEBUG] Converting to logical values (scale_factor: {})", validated.scale_factor);
                    info!("[DEBUG] - Before conversion: x={:?}, y={:?}", validated.x, validated.y);
                }
                if debug_enabled {
                    info!("[DEBUG] Converting physical values to logical values");
                    info!("[DEBUG] - Scale factor: {}", validated.scale_factor);
                    info!("[DEBUG] - Physical size: {}x{}", validated.width, validated.height);
                    if let (Some(x), Some(y)) = (validated.x, validated.y) {
                ...
                if debug_enabled {
                    ...
                    if let (Some(x), Some(y)) = (validated.x, validated.y) {
                        ...
                        info!("[DEBUG] - Will convert to: ({}, {})", x / validated.scale_factor, y / validated.scale_factor);
                    }
                }
                ...
                if debug_enabled {
                    ...
                    info!("[DEBUG] - Logical size: {}x{}", logical.width, logical.height);
        ...
        None => {
            ...
            let mut default_state = if window_label == "bb_chat" {
                if debug_enabled {
                    info!("[DEBUG] Creating chat window state with scale factor: {}", actual_scale_factor);
                }
                if debug_enabled {
                    info!("[DEBUG] Creating chat window state with scale factor: {}", actual_scale_factor);
                }
            ...
            if debug_enabled {
                info!("[DEBUG] Adjusted default state with scale factor {}:", actual_scale_factor);
                info!("[DEBUG] - Physical size: {}x{}", default_state.width, default_state.height);
                info!("[DEBUG] - Physical position: {:?},{:?}", default_state.x, default_state.y);
            }
...
#[tauri::command]
pub async fn save_window_state(window_label: String, app_handle: tauri::AppHandle, force: bool) -> Result<(), String> {
    let window = app_handle.get_webview_window(&window_label)
        .ok_or_else(|| format!("Window {} not found", window_label))?;
...
fn save_window_state_internal(window: &WebviewWindow, force: bool) {
    ...
    let save_handle = SAVE_HANDLE.get_or_init(|| {
...
fn do_save(window: &WebviewWindow) {
    ...
    if debug_enabled {
        ...
        info!("[DEBUG] System scale factor: {}", window.theme().map(|_| window.scale_factor().unwrap_or(1.0)).unwrap_or(1.0));
    }
    ...
    if debug_enabled {
        ...
        info!("[DEBUG]   * size: {}x{}", size.width as f64 / scale_factor, size.height as f64 / scale_factor);
        info!("[DEBUG]   * position: {}x{}", position.x as f64 / scale_factor, position.y as f64 / scale_factor);
    }
    ...
    if debug_enabled {
        ...
        info!("[DEBUG]   * width: {}, height: {}", validated_state.width, validated_state.height);
        info!("[DEBUG]   * x: {:?}, y: {:?}", validated_state.x, validated_state.y);
    ...
    if debug_enabled {
        ...
        info!("[DEBUG] - Physical size: {}x{}", validated_state.width, validated_state.height);
        info!("[DEBUG] - Physical position: {:?},{:?}", validated_state.x, validated_state.y);
...
#[tauri::command]
pub async fn apply_window_state(window_label: String, state: WindowState, app_handle: tauri::AppHandle) -> Result<(), String> {
    let window = app_handle.get_webview_window(&window_label)
        .ok_or_else(|| format!("Window {} not found", window_label))?;
...
pub fn apply_window_state_internal(window: &WebviewWindow, state: &WindowState) {
    ...
    if debug_enabled {
        ...
        if let Ok(size) = window.outer_size() {
            info!("[DEBUG] - Current size: {}x{} (physical pixels)", size.width, size.height);
            info!("[DEBUG] - Logical size: {}x{}", size.width as f64 / current_scale, size.height as f64 / current_scale);
        }
        if let Ok(pos) = window.outer_position() {
            info!("[DEBUG] - Current position: {}x{} (physical pixels)", pos.x, pos.y);
            info!("[DEBUG] - Logical position: {}x{}", pos.x as f64 / current_scale, pos.y as f64 / current_scale);
        }
    }
    ...
    if debug_enabled {
        ...
        info!("[DEBUG] - Size: {}x{}", validated_state.width, validated_state.height);
    }
