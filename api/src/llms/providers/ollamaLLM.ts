...
import { LLMProvider, OllamaModel, LLMCallbackType } from 'api/types/llms.types.ts';
...
class OllamaLLM extends LLM {
    ...
    private async initializeOllamaClient() {
        const ollamaHost = this.projectConfig.settings.api?.llmEndpoints?.ollama;
        if (!ollamaHost) {
            throw createError(
                ErrorType.LLM,
    ...
    private asProviderMessageType(messages: LLMMessage[]): { role: string; content: string; }[] {
    ...
    private asProviderToolType(tools: LLMTool[]): any[] {
    ...
    override async prepareMessageParams(
        ...
        const model: string = speakOptions?.model || interaction.model || OllamaModel.LLAMA2;
    ...
    public override async speakWith(
        ...
        try {
            logger.debug('llms-ollama-speakWith-messageParams', JSON.stringify(messageParams, null, 2));

            const response = await this.ollama.chat(messageParams) as OllamaChatResponse;
            ...
            const messageResponse: LLMProviderMessageResponse = {
                ...
                isTool: response.message.tool_calls !== undefined,
                messageStop: {
                    stopReason: response.message.tool_calls ? 'tool_calls' : 'stop',
                    stopSequence: null,
                },
                usage: {
