	...
	if (operations && Array.isArray(operations)) {
			...
			operations.map((op, index) => {
				if (op.editType === 'searchReplace') {
					...
					if (op.blocks_index !== undefined) {
						details += `\n${LLMTool.TOOL_STYLES_CONSOLE.base.label('Target index:')} ${
							LLMTool.TOOL_STYLES_CONSOLE.content.number(op.blocks_index)
						}`;
					}
					if (op.blocks_key) {
						details += `\n${LLMTool.TOOL_STYLES_CONSOLE.base.label('Target key:')} ${
							LLMTool.TOOL_STYLES_CONSOLE.content.data(op.blocks_key)
						}`;
					}
					if (op.blocks_position !== undefined) {
						details += `\n${LLMTool.TOOL_STYLES_CONSOLE.base.label('Insert position:')} ${
							LLMTool.TOOL_STYLES_CONSOLE.content.number(op.blocks_position)
						}`;
					}
					if (op.blocks_from !== undefined && op.blocks_to !== undefined) {
						details += `\n${LLMTool.TOOL_STYLES_CONSOLE.base.label('Move:')} ${
							LLMTool.TOOL_STYLES_CONSOLE.content.number(op.blocks_from)
						} → ${LLMTool.TOOL_STYLES_CONSOLE.content.number(op.blocks_to)}`;
					}

					return details;
				} else if (op.editType === 'range') {
					let details = `${
						LLMTool.TOOL_STYLES_CONSOLE.base.label(`Range ${op.range_rangeType} ${index + 1}:`)
					}`;

					if (op.range_location) {
						details += `\n${LLMTool.TOOL_STYLES_CONSOLE.base.label('Position:')} ${
							LLMTool.TOOL_STYLES_CONSOLE.content.number(op.range_location.index)
						}`;
					}
					if (op.range_range) {
						details += `\n${
							LLMTool.TOOL_STYLES_CONSOLE.base.label('Range:')
					...
					return `${
						LLMTool.TOOL_STYLES_CONSOLE.base.label(`Structured Data Operation ${index + 1}:`)
					} Future implementation`;
				} else {
					return `${LLMTool.TOOL_STYLES_CONSOLE.base.label(`Unknown Operation ${index + 1}:`)} ${
						LLMTool.TOOL_STYLES_CONSOLE.content.data(`Type: ${op.editType}`)
					}`;
				}
			}).join('\n\n')
		}
		...
		operationDetails = stripIndents`
			...
			blockEdits.operations.map((op, index) => {
				...
				if (op._key) {
					details += `\n${LLMTool.TOOL_STYLES_CONSOLE.base.label('Target key:')} ${
						LLMTool.TOOL_STYLES_CONSOLE.content.data(op._key)
					}`;
				}
		...
		operationDetails = `${
			LLMTool.TOOL_STYLES_CONSOLE.base.label('Structured Data Operations:')
		} ${operationCount} operations (future implementation)`;
	}
	...
	const content = stripIndents`
		...
		createIfMissing !== undefined
			? `${LLMTool.TOOL_STYLES_CONSOLE.base.label('Create if missing:')} ${
				LLMTool.TOOL_STYLES_CONSOLE.content.boolean(createIfMissing)
			}`
			: ''
	}
...
export const formatLogEntryToolResult = (
	...
	const content = stripIndents`
		${
		LLMTool.TOOL_STYLES_CONSOLE.content.status[isSuccess ? 'success' : 'error'](
