...
export async function fetchSupabaseConfig(
	...
	try {
		...
		//logger.info('AuthConfig: fetching Supabase config from BUI using: ', configUrl);
		...
		logger.error('AuthConfig: Supabase config fetch failed:', message);
		throw new ConfigFetchError(`Fetch config failed: ${message}`, 0);
	}
}
