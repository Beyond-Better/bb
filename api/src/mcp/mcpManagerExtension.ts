  ...
  if (!('listResources' in mcpManager)) {
    (mcpManager as any).listResources = async function(
      ...
      try {
        ...
        // Process the result
        return {
          resources: Array.isArray(result.resources) ? result.resources.map((r: any) => ({
            uri: r.uri || r.path || '',
            type: r.type || 'unknown',
  ...
  if (!('searchResources' in mcpManager)) {
    (mcpManager as any).searchResources = async function(
      ...
      try {
        ...
        // Process the result
        return {
          matches: Array.isArray(result.matches) ? result.matches.map((m: any) => ({
            resource: {
              uri: m.resource.uri || m.resource.path || '',
              type: m.resource.type || 'unknown',
              contentType: m.resource.contentType || m.resource.mime_type || 'application/octet-stream',
              size: m.resource.size,
              last_modified: m.resource.last_modified || m.resource.lastModified || new Date().toISOString(),
            },
            snippets: m.snippets || [],
            score: m.score || 0,
          })) : [],
      ...
      try {
        ...
        return {
          ...
          metadata: {
            ...
            size: result.metadata?.size || (typeof content === 'string' ? new TextEncoder().encode(content).length : content.length),
        ...
        throw new Error(`Failed to write resource: ${error.message}`);
      }
    };
  }
  ...
  if (!('moveResource' in mcpManager)) {
    (mcpManager as any).moveResource = async function(
      ...
      try {
        ...
        logger.error(`MCPManager: Error moving resource ${sourcePath} to ${destPath} on server ${serverId}:`, error);
        throw new Error(`Failed to move resource: ${error.message}`);
      }
    };
  }
...
function processResourceResult(result: MCPToolResult, resourcePath: string): ResourceLoadResult {
  ...
  const content = typeof result.content === 'string'
    ? result.content
    : new Uint8Array(result.content as ArrayBuffer);
