...
export class ConversationMigration {
	static async migrateProject(dataDir: string): Promise<MigrationResult> {
		...
		try {
				...
				try {
					...
					if (finalMigration.success) {
						if (finalMigration.version.from === finalMigration.version.to) {
							...
							updatedConversations.push(metadata);
						}
			...
			if (needsUpdate) {
				await ConversationMigration.saveConversationsJson(dataDir, updatedConversations);
			}
	...
	private static async migrateV2toV3(conversationDir: string): Promise<ConversationMigrationResult> {
		...
		try {
			...
			result.changes.push({
				type: 'metadata',
				path: 'metadata.json',
				details: 'Updated tokenUsageConversation and version',
			});
	...
	private static async readMetadata(path: string): Promise<ConversationMetadata> {
		try {
	...
	private static async saveMetadata(path: string, metadata: ConversationMetadata): Promise<void> {
		await Deno.writeTextFile(path, JSON.stringify(metadata, null, 2));
	}

	private static async readConversationsJson(dataDir: string): Promise<ConversationMetadata[]> {
	...
	private static async saveConversationsJson(dataDir: string, conversations: ConversationMetadata[]): Promise<void> {
		const path = join(dataDir, 'conversations.json');
