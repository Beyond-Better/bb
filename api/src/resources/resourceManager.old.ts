...
//import { DataSourceFactory } from 'api/dataSources/dataSourceFactory.ts';
...
export class ResourceManager {
	...
	/*
async loadResourceNew(resourceUri: string, options?: FileLoadOptions): Promise<any> {
  const { uriPrefix } = parseDataSourceUri(resourceUri);
  const dataSource = this.projectEditor.getDataSourceForPrefix(uriPrefix);

  if (!dataSource) {
    throw new Error(`No data source found for: ${uriPrefix}`);
  }

  const dataSourceImpl = DataSourceFactory.getDataSource(dataSource);
  return await dataSourceImpl.loadResource(resourceUri, options);
}

async listResourcesNew(dataSourceId: string, options?: any): Promise<LoadDatasourceResult> {
  ...
  const dataSourceImpl = DataSourceFactory.getDataSource(dataSource);
  return await dataSourceImpl.listResources(options);
}
 */
