...
import builtinCapabilities from '../data/modelCapabilities.json' assert { type: 'json' };
...
export class ModelCapabilitiesManager {
  ...
  public async initialize(): Promise<void> {
    ...
    try {
      ...
      logger.error(`ModelCapabilitiesManager: Failed to initialize: ${error.message}`);
      throw error;
    }
  }
  ...
  private validateCapabilities(capabilities: Record<string, Record<string, ModelCapabilities>>): void {
    ...
    for (const [provider, models] of Object.entries(capabilities)) {
      ...
      for (const [model, capabilities] of Object.entries(models)) {
        ...
        const requiredProps = ['contextWindow', 'maxOutputTokens', 'pricing', 'supportedFeatures', 'defaults', 'constraints'];
        for (const prop of requiredProps) {
          if (!(prop in capabilities)) {
            throw new Error(`Invalid capabilities for model ${provider}/${model}: missing required property ${prop}`);
          }
        }
      }
    }
   ...
   */
  public supportsFeature(provider: string, model: string, feature: keyof ModelCapabilities['supportedFeatures']): boolean {
...
export function getUserModelPreferences(
  ...
  projectConfig: any  // Using any because the exact config structure may vary
): UserModelPreferences | undefined {
  try {
    return projectConfig?.settings?.api?.llmProviders?.[provider]?.userPreferences;
  } catch (error) {
    logger.warn(`Error getting user model preferences: ${error.message}`);
    return undefined;
  }
}
